          ...
          <motion.div className="lg:col-span-4" variants={itemVariants}>
            <Card className="bg-gray-800 border-gray-700 h-full flex flex-col">
              ...
              <CardContent className="flex-grow">
                ...
                {!isEditing ? (<><Button onClick={() => setIsEditing(true)} className="w-full bg-teal-500 hover:bg-teal-600 font-bold"><Edit className="mr-2 h-4 w-4" />{userProfile && userProfile[1] ? 'Edit Profile' : 'Create Profile'}</Button><div className="mt-6 space-y-4"><h3 className="font-semibold text-lg text-white">Your Stats</h3>
                <div className="flex items-center justify-between text-sm"><span className="flex items-center gap-2 text-gray-400"><Image className="h-4 w-4" /> Memes Minted</span><Badge variant="secondary" className="bg-violet-500/20 text-violet-300">{userMemes.length}</Badge></div><div className="flex items-center justify-between text-sm"><span className="flex items-center gap-2 text-gray-400"><Star className="h-4 w-4" /> Total Movie Rentals</span><Badge variant="secondary" className="bg-pink-500/20 text-pink-300">{totalRentals}</Badge></div>
                </div></>) : (<form onSubmit={handleProfileSubmit} className="space-y-4"><div className="space-y-2"><Label htmlFor="username">Username</Label><Input id="username" value={profileData.username} onChange={(e) => setProfileData(p => ({ ...p, username: e.target.value }))} placeholder="Enter your username" required /></div>
